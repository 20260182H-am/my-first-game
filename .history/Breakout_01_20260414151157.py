...
class PermanentDebuff:
    """보스3 1회 분열 후 상시 유지되는 디버프"""
    def __init__(self, kind):
        self.kind  = kind
        self.timer = 0  # 항상 0 유지 (blinking 안 함)
...
class DebuffManager:
    ...
    mode:
      "none"        — 디버프 없음 (챕터1 일반)
      "interval"    — 쿨다운/지속 반복 (챕터2,3 일반, 보스1/2 체력 절반 이하)
      "boss3"       — 영구 1개 + interval 추가 (보스3 1회 분열 후)

    interval_params: (delay_f, duration_f, cooldown_f)
      delay_f   — 최초 시작 전 대기 프레임
      duration_f — 디버프 지속 프레임
      cooldown_f — 디버프 끝난 후 쿨다운 프레임
    """

    def __init__(self, chapter, mode="none", interval_params=None):
        self.chapter      = chapter
        self.mode         = mode
        self.active       = []      # ActiveDebuff 목록
        self.permanent    = []      # PermanentDebuff 목록 (보스3)
        self._pending_fake= False

        # 인터벌 타이머 상태
        self._iv_state    = "delay"   # delay / active / cooldown
        self._iv_timer    = 0
        self._iv_params   = interval_params or (600, 300, 600)  # 기본값
        self._iv_current  = None   # 현재 인터벌로 걸린 debuff kind
    ...
    def _update_interval(self):
        ...
        if self._iv_state == "delay":
            ...
            if self._iv_timer >= duration_f:
                ...
                # 인터벌 디버프 제거
                if self._iv_current:
                    self.active = [d for d in self.active
                                   if d.kind != self._iv_current]
                    self._iv_current = None
    ...
    def pop_popup_request(self):
        """새로 발동된 디버프 kind 반환 (있으면), 없으면 None"""
        kind = getattr(self, '_pending_popup', None)
...
class DebuffPopup:
    ...
    def draw(self, screen):
        ...
        # 페이드인 0~0.2, 유지 0.2~0.8, 페이드아웃 0.8~1.0
        if t < 0.15:
...
class SplitBlock:
    ...
    # 체력 총합: size4=4, size2=2×4=8, size1=1×16=16 → 합계 28 (분열 도중 일부 제거)
    # 체력바 max=32 (4+8+16+여유 4)
    SIZE_HP           = {4:4, 2:2, 1:1}
    ...
    def update(self):
        ...
        for b in to_split:
            ...
            if self._split_count == 1:
                self._perm_debuff_added = True  # 상시 디버프 트리거
...
class GameManager:
    ...
    def update(self):
        ...
        if self.state in ("PLAY","BOSS"):
            ...
            if self.state == "BOSS" and self.boss:
                ...
                # 보스1/2 체력 절반 이하 시 디버프 인터벌 전환
                if isinstance(self.boss, (Boss1, Boss2)):
                    if self.boss.debuff_phase_should_trigger:
                        if self.chapter == 1:
                            # 보스1: 10초 쿨다운
                            self.debuff_mgr.set_mode("interval",
                                interval_params=(0, 300, 600))
                        elif self.chapter == 2:
                            # 보스2: 5초 쿨다운
                            self.debuff_mgr.set_mode("interval",
                                interval_params=(0, 300, 300))

                # 보스3 1회 분열 후 상시 디버프 추가
                if isinstance(self.boss, Boss3):
                    if self.boss.pop_perm_debuff_request():
                        kind = random.choice(BOSS3_PERMANENT_DEBUFFS)
                        self.debuff_mgr.add_permanent(kind)
                        self.debuff_popup.show(kind)
                        # 인터벌 디버프도 시작 (상시 디버프 제외)
                        self.debuff_mgr.set_mode("boss3",
    ...
    def _on_boss_intro_done(self):
        ...
        if   self.chapter == 1:
            self.boss = Boss1()
            # 보스1 진입 시에는 아직 디버프 없음 (체력 절반 이하 시 발동)
            self.debuff_mgr = DebuffManager(self.chapter, mode="none")
        elif self.chapter == 2:
            self.boss = Boss2()
            self.debuff_mgr = DebuffManager(self.chapter, mode="none")
